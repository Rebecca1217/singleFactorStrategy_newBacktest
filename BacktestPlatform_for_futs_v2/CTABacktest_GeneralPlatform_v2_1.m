function [BacktestResult,BacktestAnalysis] = CTABacktest_GeneralPlatform_v2_1(TargetTradeList,StrategyPara,TradePara)
...
if isempty(StrategyPara)
    names = {'crossType';'freqK';'stDate';'edDate'};
else
    names = setdiff({'crossType';'freqK';'stDate';'edDate'},fieldnames(StrategyPara));
end
...
for i_fut = 1:length(fut_variety) 
    ...
    TableData = TableData(TableData.date>=StrategyPara.stDate & TableData.date<=StrategyPara.edDate,:);
